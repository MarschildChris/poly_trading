...
export class TradingService {
    ...
    async deriveOrCreateApiKey() {
        ...
        if (!created.key) {
            throw new PolymarketError(ErrorCode.AUTH_FAILED, 'Failed to create or derive API key. Wallet may not be registered on Polymarket.');
        }
        return created;
    }
    ...
    async cancelOrder(orderId) {
        ...
        return this.rateLimiter.execute(ApiType.CLOB_API, async () => {
            try {
                ...
                throw new PolymarketError(ErrorCode.ORDER_FAILED, `Cancel failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async cancelOrders(orderIds) {
        ...
        return this.rateLimiter.execute(ApiType.CLOB_API, async () => {
            try {
                ...
                throw new PolymarketError(ErrorCode.ORDER_FAILED, `Cancel orders failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async cancelAllOrders() {
        ...
        return this.rateLimiter.execute(ApiType.CLOB_API, async () => {
            try {
                ...
                throw new PolymarketError(ErrorCode.ORDER_FAILED, `Cancel all failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
