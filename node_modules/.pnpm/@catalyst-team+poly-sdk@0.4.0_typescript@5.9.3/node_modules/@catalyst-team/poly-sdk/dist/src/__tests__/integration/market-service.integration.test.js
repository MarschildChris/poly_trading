...
describe('MarketService Integration', () => {
    ...
    beforeAll(async () => {
        ...
        service = new MarketService(undefined, // gammaApi not needed for CLOB tests
        undefined, // dataApi not needed for CLOB tests
        new RateLimiter(), createUnifiedCache());
    ...
    describe('getTokenOrderbook', () => {
        it('should return sorted orderbook', async () => {
            ...
            console.log(`✓ getTokenOrderbook: ${book.bids.length} bids, ${book.asks.length} asks`);
        }, 30000);
    });
        ...
        it('should return midpoint price or handle no orderbook', async () => {
            try {
                ...
                if (!isNaN(midpoint)) {
                    ...
                    console.log(`✓ getMidpoint: NaN (no orderbook exists - expected for some tokens)`);
                }
            }
            catch {
                // API may return error if no orderbook exists
                console.log(`✓ getMidpoint: API error (no orderbook - expected for some tokens)`);
            }
        }, 30000);
    });
    describe('getSpread', () => {
        it('should return spread or handle no orderbook', async () => {
            try {
                ...
                if (!isNaN(spread)) {
                    ...
                    console.log(`✓ getSpread: NaN (no orderbook exists - expected for some tokens)`);
                }
            }
            catch {
                // API may return error if no orderbook exists
                console.log(`✓ getSpread: API error (no orderbook - expected for some tokens)`);
            }
        }, 30000);
    });
        ...
        it('should return price history array', async () => {
            try {
                ...
                if (history.length > 0) {
                    ...
                    console.log(`✓ getPricesHistory: empty (no history available for this token)`);
                }
            }
            catch {
                // API may return error for some tokens
                console.log(`✓ getPricesHistory: API error (no history - expected for some tokens)`);
            }
        }, 30000);
    });
...
//# sourceMappingURL=market-service.integration.test.js.map