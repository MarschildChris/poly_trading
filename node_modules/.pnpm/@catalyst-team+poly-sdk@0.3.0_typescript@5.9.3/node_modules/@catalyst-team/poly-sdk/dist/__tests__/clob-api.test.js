...
import { MockRateLimiter, MockCache, mockClobMarket, expectOrderbookSorted, } from './test-utils.js';
describe('ClobApiClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new ClobApiClient(new MockRateLimiter(), new MockCache());
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    describe('getProcessedOrderbook', () => {
        ...
        it('should throw error if tokens are missing', async () => {
            ...
            await expect(client.getProcessedOrderbook(mockClobMarket.conditionId)).rejects.toThrow('Missing tokens');
        });
    });
    describe('hasTradingCapabilities', () => {
        ...
        it('should return true with signer', () => {
            const clientWithSigner = new ClobApiClient(new MockRateLimiter(), new MockCache(), { signer: {} });
            expect(clientWithSigner.hasTradingCapabilities()).toBe(true);
        });
        it('should return true with credentials', () => {
            const clientWithCreds = new ClobApiClient(new MockRateLimiter(), new MockCache(), { creds: { key: 'k', secret: 's', passphrase: 'p' } });
            expect(clientWithCreds.hasTradingCapabilities()).toBe(true);
        });
    });
});
//# sourceMappingURL=clob-api.test.js.map