...
import { CTF_CONTRACT, USDC_CONTRACT, NEG_RISK_CTF_EXCHANGE, NEG_RISK_ADAPTER, USDC_DECIMALS, } from '../../clients/ctf-client.js';
...
describe('CTF Contract Verification', () => {
    ...
    describe('Position ID Calculation', () => {
        it('should calculate position ID correctly', () => {
            ...
            const collectionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'bytes32', 'uint256'], [ethers.constants.HashZero, conditionId, indexSet]));
            // Position ID = keccak256(collateralToken, collectionId)
            const positionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['address', 'bytes32'], [USDC_CONTRACT, collectionId]));
        ...
        it('should produce different position IDs for YES and NO', () => {
            ...
            const yesCollectionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'bytes32', 'uint256'], [ethers.constants.HashZero, conditionId, 1]));
            const yesPositionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['address', 'bytes32'], [USDC_CONTRACT, yesCollectionId]));
            // NO position (indexSet = 2)
            const noCollectionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'bytes32', 'uint256'], [ethers.constants.HashZero, conditionId, 2]));
            const noPositionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['address', 'bytes32'], [USDC_CONTRACT, noCollectionId]));
    ...
    describe('Balance Queries', () => {
        ...
        it('should query CTF token balance', async () => {
            ...
            const randomPositionId = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['string'], ['random-test-position']));
    ...
    describe('Market Resolution', () => {
        it('should query payout info for a real market condition', async () => {
            ...
            const response = await fetch('https://gamma-api.polymarket.com/markets?closed=true&limit=1');
...
//# sourceMappingURL=ctf-client.integration.test.js.map