...
async function main() {
  ...
  sdk.dipArb.on('started', (market) => {
    ...
    // Initialize timing for this market
    currentMarketStartTime = Date.now();
    leg1Detected = false;
    ...
    log(`│ 🔔 Signal Window: First 4 minutes from now`);
    log('└──────────────────────────────────────────────────────────┘');
  });
  ...
  const statusInterval = setInterval(() => {
    ...
    if (round) {
      if (round.phase === 'leg1_filled' && round.leg1) {
        ...
        log(`  📊 Position: None (waiting for signal in window)`);
      }
    }
  }, 30000);
