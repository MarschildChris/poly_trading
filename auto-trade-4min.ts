...
async function main() {
  ...
  const config = {
    // 交易参数 (支持命令行覆盖)
    shares: CLI_ARGS.shares,           // --shares=10
    ...
    windowMinutes: 4,        // 仅在市场开始后 4 分钟内激活策略
  ...
  // ========================================
  // Strategy Timing Control
  // ========================================
  let currentMarketStartTime: number | null = null;
  let leg1Detected = false;  // Track if leg1 signal detected in current window
  const SIGNAL_WINDOW_MS = 4 * 60 * 1000;  // 4 minutes in milliseconds
  ...
  log('╚══════════════════════════════════════════════════════════╝');
  log('');
  log('Usage: npx tsx auto-trade.ts --xrp [--dip=0.40] [--window=3000] [--timeout=60] [--shares=10] [--target=0.95]');
  log('');
  ...
  sdk.dipArb.on('started', (market) => {
    ...
    // Calculate market start time based on market end time
    // 15-minute market: if end is 1:15, start is 1:00
    const marketDurationMs = market.durationMinutes * 60 * 1000;
    const calculatedMarketStartTime = market.endTime.getTime() - marketDurationMs;
    const elapsedInMarketMs = Date.now() - calculatedMarketStartTime;
    ...
    log(`│ Start Time: ${new Date(calculatedMarketStartTime).toISOString()}`);
    log(`│ End Time: ${market.endTime.toISOString()}`);
    log(`│ Condition: ${market.conditionId.slice(0, 30)}...`);

    // Check if we're already outside the 4-minute signal window
    if (elapsedInMarketMs > SIGNAL_WINDOW_MS) {
      const minutesElapsed = Math.round(elapsedInMarketMs / 60000);
      const minutesRemaining = Math.round((market.durationMinutes * 60 * 1000 - elapsedInMarketMs) / 60000);
      log(`│ ⏸️  SIGNAL WINDOW CLOSED`);
      log(`│ Market already ${minutesElapsed}m in (${minutesRemaining}m remaining)`);
      log(`│ Waiting for next market at ${new Date(market.endTime.getTime() + 1000).toISOString().slice(11, 16)}`);
      log('└──────────────────────────────────────────────────────────┘');
      // Don't initialize signal window - skip this market
      currentMarketStartTime = null;
      leg1Detected = false;
      return;
    }

    // Initialize timing for this market (only if within signal window)
    currentMarketStartTime = calculatedMarketStartTime;
    leg1Detected = false;

    const remainingWindowSeconds = Math.round((SIGNAL_WINDOW_MS - elapsedInMarketMs) / 1000);
    log(`│ ✅ SIGNAL WINDOW ACTIVE`);
    log(`│ ${remainingWindowSeconds}s remaining in 4-minute window`);
    log('└──────────────────────────────────────────────────────────┘');
  });
  ...
  sdk.dipArb.on('signal', (signal) => {
    // If currentMarketStartTime is null, we're skipping this market - ignore all leg1 signals
    if (currentMarketStartTime === null && signal.type === 'leg1') {
      return;  // Silently ignore leg1 signals outside the window
    }

    const now = Date.now();
    const elapsedMs = currentMarketStartTime ? now - currentMarketStartTime : 0;
    const elapsedSeconds = Math.round(elapsedMs / 1000);

    // Check if we're within the 4-minute signal window for leg1
    if (signal.type === 'leg1') {
      if (elapsedMs > SIGNAL_WINDOW_MS) {
        log(`⏸️  LEG1 SIGNAL IGNORED: Outside 4-minute window (${elapsedSeconds}s elapsed)`);
        log(`   Buy ${signal.dipSide} @ ${signal.currentPrice.toFixed(4)} | Drop: ${(signal.dropPercent * 100).toFixed(1)}%`);
        return;  // Ignore this signal, don't execute
      }
      leg1Detected = true;
    }

    // Process signal (leg1 within window or any leg2)
    log('');
    log('╔══════════════════════════════════════════════════════════╗');
    if (signal.type === 'leg1') {
      log(`║  ✅ LEG1 SIGNAL (${elapsedSeconds}s): Buy ${signal.dipSide} @ ${signal.currentPrice.toFixed(4)}`);
  ...
  const statusInterval = setInterval(() => {
    ...
    // Calculate signal window status
    let windowStatus = '';
    if (currentMarket) {
      if (currentMarketStartTime === null) {
        // Skipping this market, show time until next market
        const timeUntilNextMarket = Math.round((currentMarket.endTime.getTime() - Date.now()) / 1000);
        if (timeUntilNextMarket > 0) {
          windowStatus = `⏸️  Skipping market | Next market in ${timeUntilNextMarket}s`;
        } else {
          windowStatus = `⏸️  Waiting for next market to start...`;
        }
      } else if (currentMarketStartTime) {
        const elapsedMs = Date.now() - currentMarketStartTime;
        const elapsedSeconds = Math.round(elapsedMs / 1000);
        if (elapsedMs < SIGNAL_WINDOW_MS) {
          const remainingSeconds = Math.round((SIGNAL_WINDOW_MS - elapsedMs) / 1000);
          windowStatus = `🔔 Window: ${elapsedSeconds}s/${Math.round(SIGNAL_WINDOW_MS / 1000)}s (${remainingSeconds}s left)`;
        } else {
          windowStatus = `⏸️  Window CLOSED (${elapsedSeconds}s ago) - Waiting for next market`;
        }
      }
    }
    ...
    if (currentMarket) {
      ...
      log(`[Status #${statusCount}] Market: ${currentMarket.underlying} | ${timeLeftStr} | ${windowStatus} | Signals: ${stats.signalsDetected} | L1: ${stats.leg1Filled} | L2: ${stats.leg2Filled}`);
    } else {
    ...
    if (round) {
      if (round.phase === 'leg1_filled' && round.leg1) {
        ...
        const statusMsg = currentMarketStartTime === null ? 'waiting for next market' : 'waiting for signal in window';
        log(`  📊 Position: None (${statusMsg})`);
      }
    }
  }, 30000);
